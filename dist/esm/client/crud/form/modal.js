...
export function ModalForm({ modalId: thisModalId, mode, title, fields, schema, endpoint, parseFetchedData, mutationParams, buttonLabel = false, onSuccess, cols, }) {
    const { id, open, modalId, closeModal, defaultValues } = useModalForm();
    const Struct = useStructUI();
    if (!endpoint)
        return null;
    const isOpen = open && modalId === thisModalId;
    return (_jsx(Struct.Dialog.Root, { open: isOpen, onOpenChange: closeModal, children: _jsxs(Struct.Dialog.Content, { className: "sm:w-[95%] sm:max-w-3xl max-h-[95%] overflow-y-auto", children: [_jsx(Struct.Dialog.Header, { children: _jsx(Struct.Dialog.Title, { children: title || (id ? "Editar" : "Novo") }) }), _jsx(ModelForm, { mode: mode || id ? "edit" : "register", endpoint: endpoint, id: id, schema: schema, fields: fields, mutationParams: mutationParams, parseFetchedData: parseFetchedData, buttonLabel: buttonLabel, cols: cols, defaultValues: defaultValues, redirectAfterRegister: false, onAfterSubmit: (response) => {
                        closeModal();
                        onSuccess?.(response);
                    } })] }) }));
}
