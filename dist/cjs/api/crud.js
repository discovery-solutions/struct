...
class CRUDController {
    constructor(model, options = {}) {
        ...
        this.GET = (0, utils_1.withSession)(async ({ user }, req, { params }) => {
            await this.promise;
            let { id } = (await params) || {};
        ...
        this.POST = (0, utils_1.withSession)(async ({ user }, req) => {
            await this.promise;
            let body;
            try {
        ...
        this.PATCH = (0, utils_1.withSession)(async ({ user }, req, { params }) => {
            await this.promise;
            const [id] = (await params).id;
            let body = this.options.customParser
                ? await this.options.customParser(req)
                : await req.json();
            const original = await this.service.findOne({ _id: new mongoose_1.default.Types.ObjectId(id) });
            if (!original)
                return Response.json({ message: "Not Found" }, { status: 404 });
            ...
            const updated = await this.service.updateOne({ _id: new mongoose_1.default.Types.ObjectId(id) }, { ...body, updatedAt: new Date() });
            if (!updated)
                return Response.json({ message: "Not Found" }, { status: 404 });
        ...
        this.DELETE = (0, utils_1.withSession)(async ({ user }, _, { params }) => {
            await this.promise;
            ...
            if (this.options.softDelete) {
                ...
                await this.service.deleteOne({ _id: new mongoose_1.default.Types.ObjectId(id) });
            }
        ...
        this.parseFilters = (filters) => {
            ...
            const convertObjectIds = (obj) => {
                for (const [key, value] of Object.entries(obj)) {
                    ...
                    else if (key === "id" && (0, mongoose_1.isObjectIdOrHexString)(value)) {
        ...
        this.promise = config_1.Struct.config?.database?.startConnection();
