...
**Arquivo:** `skills/confirm-dialog.md`  
**Descrição:** Componente React para diálogos de confirmação. Executa automaticamente requisições HTTP (DELETE/PATCH/POST) após confirmação do usuário. Ideal para ações destructivas como delete.

#### 11. FieldRender

**Arquivo:** `skills/field-render.md`  
**Descrição:** Componente React para renderizar campos de formulário sem validação automática. Perfeito para formulários customizados, wizards multi-step, ou quando você precisa de controle total sobre o submit.

#### 12. SearchHeader

**Arquivo:** `skills/search-header.md`  
**Descrição:** Componente React para header de páginas de listagem. Inclui campo de busca e botão "Adicionar Novo". Integração com ModalForm para abrir modais de criação.

#### 13. StructUI

**Arquivo:** `skills/struct-ui.md`  
**Descrição:** Provider de configuração de componentes UI. Fornece componentes base (Button, Input, Dialog, etc), sistema de toast e mapeamento de tipos de campos. Deve ser configurado uma vez no topo da aplicação.
...
| Criar diálogo de confirmação       | confirm-dialog.md  |
| Renderizar campos customizados     | field-render.md    |
| Header com busca e botão adicionar | search-header.md   |
| Configurar componentes UI da lib   | struct-ui.md       |
